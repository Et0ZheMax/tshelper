...
class UserButton(ttk.Button):
    ...
    def __init__(self, master, user, available, app, *args, **kwargs):
        ...
        # По умолчанию используем стиль "TButton" (с синей обводкой), а при поиске – Green/Red.
        self.style = "TButton"
    ...
    def open_ps_terminal(self):
        try:
            # Запускаем PowerShell в новом окне с командой Enter-PSSession с указанным именем ПК
    ...
    def update_style(self, available):
        self.available = available
        # Если в поле поиска введено 3+ символов, используем пинговые стили, иначе базовый
...
class MainWindow:
    ...
    def __init__(self, master):
        ...
        # Стили для пинговых результатов (используются только при поиске)
        self.style.configure("Green.TButton", background="#04d1b9", foreground="black")
        self.style.configure("Red.TButton", background="#6666ff", foreground="black")
        ...
        # Для хранения AD-учетных данных (до конца сессии) – пытаемся загрузить их из настроек
        self.ad_credentials = {}
        ...
        # Всплывающая подсказка удалена

        # Заполняем кнопками (если список пуст, остается пустым до AD Sync)
    ...
    def create_widgets(self):
        ...
        # В меню "Файл" добавляем окно для ввода AD-данных (настройки)
    ...
    def open_edit_window(self, user):
        # Открываем окно редактирования для записи user
    ...
    def open_settings(self):
        # Окно настроек для ввода учетных данных AD
        settings_window = tk.Toplevel(self.master)
        settings_window.title("Настройки")
        # Восстанавливаем геометрию окна, если она сохранена
        geom = self.settings_manager.get_setting("settings_window_geometry")
        if geom:
            settings_window.geometry(geom)
        # Привязываем закрытие с сохранением геометрии под ключом "settings_window_geometry"
    ...
    def on_toplevel_close(self, window, config_key):
        # Если config_key указан, сохраняем геометрию окна
        if config_key:
    ...
    def ad_sync(self):
        # Если AD-данные уже сохранены, используем их, иначе запрашиваем через окно настроек
    ...
    def perform_ad_sync(self, username, password):
        ...
        if not ad_users:
            return
        # Сравнение по ФИО (имя) без учета регистра

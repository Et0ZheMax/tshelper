...
- Для ADHelper.py поддержана генерация приветственного ODT после успешного создания пользователя. Шаблон берётся из файла `New User.odt` рядом со скриптом. Предпочтительный режим — плейсхолдеры `{{LOGIN}}`, `{{DOMAIN_LOGIN}}`, `{{EMAIL}}`, `{{PASSWORD}}` в тексте документа; fallback-режим умеет заменять значения в строках с префиксами `Логин:`, `Доменный логин:`, `Ваш Email:`, `Временный пароль:`.
